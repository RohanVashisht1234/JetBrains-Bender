...
final class BendSpellcheckingStrategy extends SpellcheckingStrategy {
  ...
  public @NotNull Tokenizer<?> getTokenizer(PsiElement element) {
    if (element instanceof PsiComment) {
      return new BendCommentTokenizer();
    }

    if (element instanceof BendProperty) {
      return new BendPropertyTokenizer();
    }

    return EMPTY_TOKENIZER;
  }

  private static class BendCommentTokenizer extends Tokenizer<PsiComment> {
  ...
  private static class BendPropertyTokenizer extends Tokenizer<BendProperty> {

    public void tokenize(@NotNull BendProperty element, @NotNull TokenConsumer consumer) {
      //Spell check the keys and values of properties with different splitters
      final ASTNode key = element.getNode().findChildByType(BendTypes.KEY);
      ...
      final ASTNode value = element.getNode().findChildByType(BendTypes.VALUE);
